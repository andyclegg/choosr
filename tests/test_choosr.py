...
class TestConfigurationHandling:
    ...
    def test_load_config_valid_yaml(self):
        ...
        config_data = {
            "browser_profiles": {
                "Default": {"browser": "chrome", "profile_id": "Default", "is_private": False}
            },
    ...
    def test_save_url_match(self):
        ...
        with patch.object(choosr, 'load_config', return_value=existing_config), \
...
class TestBrowserManagement:
    ...
    def test_launch_browser_success(self):
        ...
        with patch.object(choosr, '_find_profile_by_name', return_value=(test_profile, mock_browser)):
            choosr.launch_browser("Test Profile", "https://example.com")
            
            mock_browser.launch.assert_called_once_with(test_profile, "https://example.com")
    ...
    def test_launch_browser_fallback_to_chrome(self):
        ...
        with patch.object(choosr, '_find_profile_by_name', return_value=(None, None)), \
...
class TestConfigInitialization:
    ...
    def test_init_config_file_exists(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            with patch('builtins.print') as mock_print:
                ...
                assert any("Config file already exists" in msg for msg in printed_messages)
    ...
    def test_init_config_create_new(self):
        ...
        with patch('os.path.exists', return_value=False), \
             patch.object(choosr, 'get_all_browser_profiles', return_value=mock_profiles), \
             patch('builtins.open', mock_open()) as mock_file:
...
class TestUrlHandling:
    ...
    def test_handle_url_with_existing_match(self):
        ...
        config = {
            "browser_profiles": {"Work Profile": {"browser": "chrome", "profile_id": "work"}},
            "urls": [{"match": "work.com", "profile": "Work Profile"}]
        }
        
        with patch.object(choosr, 'load_config', return_value=config), \
             patch.object(choosr, 'launch_browser') as mock_launch:
            
            choosr.handle_url("https://mail.work.com")
            
            mock_launch.assert_called_once_with("Work Profile", url="https://mail.work.com")

    def test_handle_url_no_match_with_gui(self):
        """Test handling URL with no match, showing GUI selector."""
        config = {
            "browser_profiles": {"Default": {"browser": "chrome", "profile_id": "default"}},
            "urls": []
        }
        
        with patch.object(choosr, 'load_config', return_value=config), \
             patch.object(choosr, 'show_qt_profile_selector', return_value=("Default", "*.example.com", True)), \
...
class TestMainFunction:
    ...
    def test_main_init_command(self):
        ...
        with patch('sys.argv', ['choosr', 'init']), \
    ...
    def test_main_url_command(self):
        ...
        with patch('sys.argv', ['choosr', 'url', 'https://example.com']), \
             patch.object(choosr, 'initialize_browsers'), \
    ...
    def test_main_invalid_command(self):
        ...
        with patch('sys.argv', ['choosr', 'invalid']), \
             patch.object(choosr, 'initialize_browsers'), \
             patch('argparse.ArgumentParser.print_help') as mock_help, \
             patch('sys.exit') as mock_exit:
...
class TestErrorHandling:
    ...
    def test_yaml_write_error_handler(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert any("YAML serialization error" in str(call) for call in mock_print.call_args_list)
